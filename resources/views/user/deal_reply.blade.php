...
@php
    $name =$add->from_user->name ;
    $ff=\Auth::user()->name;
    if($name == $ff){
        $name1 = $add->to_user->name;
        $username1 = $add->to_user->username;
        $note_to = $add->to_user->id;
    }else{
        $name1 = $add->from_user->name;
        $username1 = $add->from_user->username;
    ...
    <div class="container">
        <div class="row">
                ...
                <div class="personal-detail">
                    <div class="personal-detail-inner col-md-12">
                        ...
                        <div class="personal-detail-main">
                            ...
                            @if($add->advertise->add_type == 1 && $add->advertise->allow_id == 1)
                        ...
                        <div class="infromation">
                            ...
                            <div class="row pt-20">
                                <div class="col-lg-6 totl-bit">
                                    ...
                                    <span>@if($add->advertise->add_type == 1 && $add->advertise->allow_email == 1) {{$user->email}} @else Verified @endif</span>
                                </div>
                                ...
                                <div class="col-lg-6 totl-bit">
                                    ...
                                    <span>@if($add->advertise->add_type == 1 && $add->advertise->allow_phone == 1) {{$user->phone}} @else Verified @endif</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-6">
                ...
                <div class="message-right">
                    ...
                    <div class="accordion">
                        ...
                        <div class="accordion__body is-active">
                            @if($name1 == $name)
                            <p>You are {{$add->add_type == 2 ? 'buying':'selling' }} {{ number_format((float)$add->coin_amount , 8, '.', '')  }}
                                {{$add->gateway->currency}} with {{$add->amount_to}} {{$add->currency->name}}
                                {{$add->add_type == 2? 'from':'to' }} {{$name1}} at the price of {{$add->price}} {{$add->currency->name}}.</p>                           
                            @else
                            <p>You are {{$add->add_type == 1 ? 'buying':'selling' }} {{ number_format((float)$add->coin_amount , 8, '.', '')  }}
                                {{$add->gateway->currency}} for {{$add->amount_to}} {{$add->currency->name}}
                                {{$add->add_type == 1? 'from':'to' }} {{$name1}} at the price of {{$add->price}} {{$add->currency->name}}.</p>
                            @endif
                        </div>
                ...
                @if($add->status == 0 || $add->status == 9 || $add->status == 11)
                    <div class="you-paid releaseBtc">
                        <div class="content text-center">
                            <p class="pb-10">
                            <span>Make sure you got paid, then click the button "Release BTC"</span>
                        </p>
                        <p style="color: red">After you click Release BTC, {{ $add->coin_amount  }} {{$add->gateway->currency}} will be transferred from your wallet</p>

                        <div class="button-area"> <a class="paid-btn" type="button" data-bs-toggle="modal" data-bs-target="#paidModal"> Release BTC </a>
                        </div>
                        </div>
                    </div>
                @endif
                @if($add->status == 0 || $add->status == 9 || $add->status == 11)
                    <div class="you-paid">
                        <div class="content text-center DisputeButton">
                            <p class="pb-10">
                            <span>If you want to dispute the deal with {{$username1}}, then click "Dispute Request"</span>
                        </p>
                        
                        <div class="button-area DisputeButton"> <a class="paid-btn" type="button" data-bs-toggle="modal" data-bs-target="#cancelModal" id="rrrrr"> Dispute Request </a>
                        </div>
                        </div>
                    </div>
                @endif
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Confirm Paid<strong> From {{$username1}}</strong></h4>
            </div>

            <form method="post" action="{{route('confirm.paid')}}" onsubmit="return confirmRelease()">
                @csrf

                <div class="alert alert-danger notPaid">
                    Don't release coin before you get paid
                </div>
                ...
                <div class="modal-body">
                    ...
                    <h6 style="color: #0b2e13">
                        {{$username1}} want to {{$add->add_type == 1 ? 'sell':'buy' }} {{ $add->coin_amount  }} {{$add->gateway->currency}} with {{$add->amount_to}} {{$add->currency->name}} {{$add->add_type == 1 ? 'to':'from' }} you .
                        And confirmed you are paid.
                    </h6>
                </div>
                <input type="hidden" name="status" value="{{$add->id}}" />
                <div class="modal-footer">
                    <button type="submit" id="releaseBTC" name="submit" value="{{$add->id}}" class="btn btn-primary pull-right">Release BTC</button>
...
<div id="cancelModal1" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" style="color: red">Confirm Cancel Request<strong> from {{$username1}}</strong></h4>
            </div>
            ...
            <form method="post" action="{{route('confirm.cancel')}}">
                ...
                <div class="modal-body">
                    {{csrf_field()}}
                    <h6 style="color: #0b2e13">
                        {{$username1}} want to {{$add->add_type == 2 ? 'sell':'buy' }} {{ $add->coin_amount }} {{$add->gateway->currency}} with {{$add->amount_to}} {{$add->currency->name}} {{$add->add_type == 1? 'from':'to' }} you .
...
<div id="cancelModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" style="color: red">Confirm Dispute Request<strong> from {{$username1}}</strong></h4>
            </div>
            ...
            $.ajax({
                ...
                success: function(data1) {
                    ...
                    if (data1.status == 0) {
                        ...
                        $('#statusP').html('Processing');
                        $('.releaseBtc').show();
                        $('.DisputeButton').show();
                    }

                    if (data1.status == 1) {
                        $('#statusP').html('Paid Complete');
                        $('.timer').hide();
                    ...
                    if (data1.status == 9) {
                        ...
                        console.log(data1.dispute_timer)
                        var date = new Date(); // some mock date
                        var milliseconds = Math.round(date.getTime() / 1000);
                        var time = milliseconds - data1.dispute_timer;
                    ...
                    if (data1.status == 10) {
                        $('#statusP').html('Dispute');
                        // $('.releaseBtc').show(); 
                    }
                    if (data1.status === 2) {
                        $('#statusP').html('Cancelled');
                        $('.timer').hide();

                    }
                    if (data1.status == 21) {
                        $('#statusP').html('Automatically Cancelled');
                        $('.timer').hide();

                    }
                    if (data1.status == 11) {
                        $('#statusP').html('On Hold');
                        $('.DisputeButton').show();
                        $('.releaseBtc').show();
                        $('.timer').hide();
                    }
        ...
        $('#submit').click(function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // console.log(created_at);
                    // console.log(moment(new Date(created_at)).format("YYYY-MM-DD hh:mm:ss a"));
                    string += created_at + '</span></div>';// console.log(string)
                    $('#oww').prepend(string);
                    $("#preloader").hide();
                }
