    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-4 balane">
                <p><span>Edit Coin Trade Advertisement</span></p>
            </div>
        </div>
    </div>
</section>
...
<section class="pt-60 create-add">
    <div class="container">
        ...
        <form action="{{route('sell.buy.update', ['advertise' => $advertise->id, 'username' =>  auth()->user()->username])}}" method="post">
            @csrf
            @method('put')
            @if (count($errors) > 0)
            <div class="col-md-12">
            ...
            <div class="form-div pb-30 ">
                ...
                <div class="form-row Category-span1 create-add">
                    <div class="form-group col-md-6 select-cat1">
                        ...
                        <select required name="add_type" id="add_type" class="form-control coin input-control">
                            <!-- <option disabled value="" selected>Select Buy / Sell</option> -->
                            <option {{$advertise->add_type == 1? 'selected':''}} value="sell">Sell</option>
                            <option {{$advertise->add_type == 2? 'selected':''}} value="buy">Buy</option>
                        </select>
                    </div>
                    <div class="form-group col-md-6 select-cat1">
                        <span for="status" class="Select-control">Status</span>
                        <select required name="status" id="status{{$advertise->id}}" class="form-control status input-control" data-id="{{$advertise->id}}">
                            <option value="0" {{$advertise->status == '0'? 'selected':''  }}> Inactive </option>
                            <option value="1" {{$advertise->status == '1'? 'selected':''  }}> Active </option>
                            <option value="2" {{$advertise->status == '2'? 'selected':''  }}> On Vacation </option>
                        </select>
                    </div>
                </div>
                <div class="form-row Category-span1 create-add">
                    <div class="form-group col-md-6 select-cat1">
                        ...
                        <select required name="category_id" id="category_id" class="form-control input-control">
                            ...
                            <option value="{{$data->id}}" {{ $advertise->category_id == $data->id ? 'selected':'' }}>{{$data->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-md-6 select-cat1">
                        ...
                        <select required name="crypto_id" id="crypto_id" class="form-control input-control">
                            ...
                            <option value="{{$data->id}}" {{$advertise->method_id == $data->id? 'selected':''  }}>{{$data->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-row Category-span1 create-add">
                    ...
                    <div class="form-group col-md-6 select-cat1">
                        ...
                        <select required name="currency" id="currency" class="form-control input-control">
                            <option disabled value="">Select Currency</option>
                            @foreach($currency as $data)
                            <option value="{{ $data->id }}" {{$advertise->currency_id == $data->id? 'selected':''  }}>
                                {{$data->name}}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="form-row Category-span1 create-add">
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control input-control" type="number"  name="margin" step="any"
                            id="margin" placeholder="Margin" value="{{$advertise->margin}}">
                        
                    </div>
                
                    <div class="form-group col-md-6">
                        <span for="price" class="Select-control"> Price Equation</span>
                        <input class="form-control input-control" type="text" id="price" name="price1" value="{{$advertise->price}}"  readonly>
                        <input type="hidden" name="price" value="{{$advertise->price}}" id="price1">
                    </div>
                </div>
                ...
                <div class="form-div2">
                    <div class="form-div pb-30">
                        ...
                        <div class="form-row Category-span1 create-add">
                            <div class="form-group col-md-12">
                                ...
                                <input type="text" name="description" class="form-control input-control" value="{{$advertise->description}}" id="description" aria-describedby="emailHelp" maxlength="25" placeholder="Write Your Description (Maxx 25 Characters)">
                            </div>
                        ...
                        <div class="form-row Category-span1 create-add">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" name="min_amount" value="{{$advertise->min_amount}}" id="min_amount" class="form-control input-control" aria-describedby="emailHelp" placeholder="Min. Amount">
                            ...
                            <div class="form-group col-md-6 max">
                                ...
                                <input type="text" name="max_amount" id="max_amount" value="{{$advertise->max_amount}}" class="form-control input-control" aria-describedby="emailHelp" placeholder="Maxx. Amount">
                                <a class="currency-lbl">{{ $general->currency }}</a>
                        ...
                        <div class="form-row Category-span1 create-add">
                            <div class="form-group col-md-6">
                                ...
                                <textarea class="form-control input-control1" name="term_detail"
                                    rows="5">{!! $advertise->term_detail !!}</textarea>
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <textarea class="form-control input-control1" name="payment_detail"
                                    rows="5">{!! $advertise->payment_detail !!}</textarea>
                            </div>
                        </div>
                        <div class="form-row Category-span1 create-add">
                            <div class="form-group col-md-6">
                                ...
                                <ul class="unstyled centered">
                                    <li>
                                    <input class="styled-checkbox" name="allow_email" type="checkbox" value="1" id="defaultCheck1" @if(old('allow_email') == 1 || $advertise->allow_email == 1) checked @endif>
                                    <label for="defaultCheck1">Allow me to see your Email Address</label>
                                    </li>
                                    <li>
                                    <input class="styled-checkbox" name="allow_phone" type="checkbox" value="1" id="defaultCheck2" @if(old('allow_phone') == 1 || $advertise->allow_phone == 1) checked @endif>
                                    <label for="defaultCheck2">Allow me to see your phone number</label>
                                    </li>
                                    
                                    <li>
                                    <input class="styled-checkbox" name="allow_id" type="checkbox" value="1" id="defaultCheck3" @if(old('allow_id') == 1 || $advertise->allow_id == 1) checked @endif>
                                ...
                                <textarea class="form-control input-control1" name="init_message" placeholder="Type your optional welcome message/instructions here..."
                                    rows="5">{!! $advertise->init_message !!}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="footer-terms-ul">
                        ...
                        <button type="submit" class="btn btn-primary publish">Update Advertise</button>
                </div>
        </form>
